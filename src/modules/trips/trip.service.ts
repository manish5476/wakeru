...
interface UserInfo {
  userId: string; // 🐛 FIXED: Aligned with the controller (was 'uid')
  displayName: string;
  photoURL?: string;
}
...
export const createTrip = async (
  ...
  if (initialStop) {
    stopData = buildStopSubdoc(initialStop, creator.userId, 0);
  } else {
...
export const addStop = async (
  ...
  const order = input.order !== undefined ? input.order : trip.stops.length;

  const newStopData = buildStopSubdoc(input, creatorUserId, order);
...
export const addMember = async (
  ...
  // Leverage the robust findActive method on the User model
  // It handles checking by email, UUID (_id), or firebaseUid
  const user = await User.findActive(targetUserId);
  ...
  // Ensure we compare against firebaseUid which trip members use
  const existingMember = trip.members.find(
    (m) => m.userId === user.firebaseUid && !m.isActive
  );
  ...
  if (existingMember) {
    ...
    trip.members.push({
      userId: user.firebaseUid, // Explicitly enforce firebaseUid per trip design
...
// ─────────────────────────────────────────────────────────────────────────────
// CACHED TOTAL UPDATES (called by ExpenseService)
// ─────────────────────────────────────────────────────────────────────────────
...
export const incrementStopTotals = async (
  ...
  await Trip.findOneAndUpdate(
    { _id: tripId, 'members.userId': payerUserId },
