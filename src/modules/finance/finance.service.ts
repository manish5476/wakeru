...
export class FinanceService {
  ...
  /**
   * Dedicated Monthly Finance KPI Overview
   * Aggregates real backend data for current/specified month:
   * - Total Transactions, Total Expenses, You Paid, Your Share
   * - You Lent, You Borrowed, Settled, Pending, Net Balance
   * - Active People count and Top People Breakdown
   */
  static async getMonthlyFinanceOverview(userId: string, monthParam?: string) {
    const now = new Date();
    ...
    if (monthParam && /^\d{4}-\d{2}$/.test(monthParam)) {
      ...
      targetMonthIdx = now.getUTCMonth();
    }

    const startDate = new Date(Date.UTC(targetYear, targetMonthIdx, 1, 0, 0, 0, 0));
    const endDate = new Date(Date.UTC(targetYear, targetMonthIdx + 1, 0, 23, 59, 59, 999));
    ...
    const formattedMonth = `${targetYear}-${String(targetMonthIdx + 1).padStart(2, '0')}`;

    // Parallel fetching of expenses, personal transactions, and debts
    const [monthlyExpenses, monthlyTransactions, debts, userDoc] = await Promise.all([
      Expense.find({
        $or: [
          { paidBy: userId },
          { 'splits.userId': userId },
      ...
      }).select('displayName email baseCurrency').lean(),
    ]);

    const baseCurrency = userDoc?.baseCurrency || 'INR';

    let totalExpenses = 0;
    let amountPaidByMe = 0;
    let myShare = 0;
    let amountLent = 0;
    let amountBorrowed = 0;
    let settledAmount = 0;

    const peopleMap = new Map<string, { userId?: string; name: string; net: number }>();

    // 1. Process Group / Trip Expenses
    monthlyExpenses.forEach((exp: any) => {
      const expAmount = exp.amountBase || exp.amountLocal || 0;
      totalExpenses += expAmount;

      const isPayer = exp.paidBy === userId;

      if (isPayer) {
        amountPaidByMe += expAmount;

        exp.splits?.forEach((s: any) => {
          const splitAmt = s.amountBase || s.amountLocal || 0;
          if (s.userId === userId) {
            myShare += splitAmt;
          } else {
            if (s.isPaid) {
              settledAmount += splitAmt;
            } else {
              amountLent += splitAmt;
              const personKey = s.userId || s.displayName || 'Friend';
              const entry = peopleMap.get(personKey) || {
                userId: s.userId,
                name: s.displayName || 'Friend',
                net: 0,
              };
              entry.net += splitAmt; // they owe you (+)
              peopleMap.set(personKey, entry);
            }
          }
        });
      } else {
        ...
        if (mySplit) {
          const mySplitAmt = mySplit.amountBase || mySplit.amountLocal || 0;
          myShare += mySplitAmt;

          if (mySplit.isPaid) {
            settledAmount += mySplitAmt;
          } else {
            amountBorrowed += mySplitAmt;
            const personKey = exp.paidBy || exp.paidByName || 'Friend';
            const entry = peopleMap.get(personKey) || {
              userId: exp.paidBy,
              name: exp.paidByName || 'Friend',
              net: 0,
            };
            entry.net -= mySplitAmt; // you owe them (-)
            peopleMap.set(personKey, entry);
          }
        }
      }
    ...
    // 2. Process Personal Direct Transactions (non-auto-generated)
    const directTransactions = monthlyTransactions.filter((t: any) => !t.isAutoGenerated);
    directTransactions.forEach((t: any) => {
      if (t.type === 'expense') {
        totalExpenses += t.amount;
        amountPaidByMe += t.amount;
        myShare += t.amount;
    ...
    // 3. Process Personal Lending / Debts
    debts.forEach((d: any) => {
      const amt = Number(d.amount) || 0;
      const repaid = Number(d.repaidAmount) || 0;
      const outstanding = Number(d.outstandingAmount) || Math.max(0, amt - repaid);

      const isCurrentMonthDebt = d.createdAt >= startDate && d.createdAt <= endDate;

      if (d.type === 'lent') {
        if (d.status !== 'settled') {
          amountLent += outstanding;
          const key = d.friendUserId || d.name || 'Friend';
          const entry = peopleMap.get(key) || {
            userId: d.friendUserId,
            name: d.name || 'Friend',
            net: 0,
          };
          entry.net += outstanding;
          peopleMap.set(key, entry);
        }
        if (repaid > 0 && isCurrentMonthDebt) {
          settledAmount += repaid;
        }
      } else if (d.type === 'borrowed') {
        if (d.status !== 'settled') {
          amountBorrowed += outstanding;
          const key = d.friendUserId || d.name || 'Friend';
          const entry = peopleMap.get(key) || {
            userId: d.friendUserId,
            name: d.name || 'Friend',
    ...
    const pendingAmount = Math.round((amountLent + amountBorrowed) * 100) / 100;
    const netBalance = Math.round((amountLent - amountBorrowed) * 100) / 100;
    const totalTransactionsCount = monthlyExpenses.length + directTransactions.length;

    const peopleBreakdown = Array.from(peopleMap.values())
      .filter((p) => Math.abs(p.net) > 0.01)
      .map((p) => ({
        userId: p.userId,
        name: p.name,
        amount: Math.abs(Math.round(p.net * 100) / 100),
        direction: p.net >= 0 ? ('they_owe_you' as const) : ('you_owe_them' as const),
    ...
    return {
      ...
      amountPaidByMe: Math.round(amountPaidByMe * 100) / 100,
      myShare: Math.round(myShare * 100) / 100,
      amountLent: Math.round(amountLent * 100) / 100,
      amountBorrowed: Math.round(amountBorrowed * 100) / 100,
      settledAmount: Math.round(settledAmount * 100) / 100,
      pendingAmount,
      netBalance,
      activePeopleCount: peopleBreakdown.length,
      peopleBreakdown: peopleBreakdown.slice(0, 10),
    };
  }
