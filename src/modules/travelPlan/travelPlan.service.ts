...
class TravelPlanService {
  ...
  // Helper to convert partial objects to MongoDB atomic $set dot notation
  // e.g. { airline: 'Delta', cost: 500 } -> { 'flightDetails.$.airline': 'Delta', 'flightDetails.$.cost': 500 }
  ...
  async getOrCreatePlan(tripId: string): Promise<ITravelPlan> {
    ...
    if (!plan) {
      plan = await TravelPlan.create({
        ...
        'budgetOverview.currency': trip.baseCurrency,
      ...
      await plan.save(); // Utilizing pre-save hook to calculate progress and remaining budget
    }
  ...
  async updateBudget(tripId: string, budgetData: Partial<ITravelPlan['budgetOverview']>): Promise<ITravelPlan> {
    ...
    // Save to trigger hooks for remaining calculation
    await plan.save(); 
    return plan;
  }
  ...
  // Using Mongoose's DocumentArray typing to safely retrieve and modify subdocuments natively
  async toggleChecklistItem(tripId: string, itemId: string): Promise<ITravelPlan> {
    ...
    await plan.save(); // Important: saving parent doc triggers pre-save progress hooks
    return plan;
  }

  // Using MongoDB atomic updates directly - far safer for concurrent user edits!
  async updateChecklistItem(tripId: string, itemId: string, updates: Partial<IChecklistItem>): Promise<ITravelPlan> {
    ...
    // Explicit save to trigger hook recalculations for progress bar
    await plan.save();
    return plan;
  }
  ...
  async addItineraryDay(tripId: string, dayData: Partial<IItineraryDay>): Promise<ITravelPlan> {
    ...
    plan.itinerary.push({ ...dayData, day: dayData.date ? `Day ${dayNumber}` : '' } as any);
    await plan.save();
    return plan;
  }
  ...
  async generateItineraryFromStops(tripId: string): Promise<ITravelPlan> {
    ...
    trip.stops.forEach((stop: any, idx: number) => {
      plan.itinerary.push({
        ...
        transport: '', accommodation: '', meals: {}, notes: stop.notes || '',
      } as any);
    });
  ...
  // ───────────────────────────────────────────────────────────────────────────
  // Refactored to utilize a standard atomic subdoc pattern

  async addFlight(tripId: string, data: Omit<IFlightDetail, '_id'>): Promise<ITravelPlan> {
    const plan = await TravelPlan.findOneAndUpdate({ tripId }, { $push: { flightDetails: data } }, { new: true });
    if (!plan) throw new AppError('Plan not found', 404);
    await plan.save(); // triggers progress hook
    return plan;
  }
  ...
  async deleteFlight(tripId: string, id: string): Promise<ITravelPlan> {
    const plan = await TravelPlan.findOneAndUpdate({ tripId }, { $pull: { flightDetails: { _id: id } } }, { new: true });
    if (!plan) throw new AppError('Plan not found', 404);
    await plan.save();
    return plan;
  }
  ...
  async addAccommodation(tripId: string, data: Omit<IAccommodationDetail, '_id'>): Promise<ITravelPlan> {
    const plan = await TravelPlan.findOneAndUpdate({ tripId }, { $push: { accommodationDetails: data } }, { new: true });
    if (!plan) throw new AppError('Plan not found', 404);
    await plan.save();
    return plan;
  }
  ...
  async addPackingItem(tripId: string, data: { category: string, name: string, quantity?: number, priority?: string }): Promise<ITravelPlan> {
    ...
    // If category didn't exist, push entirely new category object
    if (!plan) {
  ...
  // Advanced atomic update using arrayFilters to safely toggle an item deeply nested in categories
  ...
  async getPlanningProgress(tripId: string) {
    ...
    if (!plan) {
      return { overall: 0, checklist: 0, packing: 0, bookings: 0, documents: 0, tripStatus: trip?.status || 'planning', daysUntilTrip, urgentItems: [], expiringDocs: [], upcomingFlights: [] };
    }
