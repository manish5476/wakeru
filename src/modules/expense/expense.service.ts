...
export const getMyExpenses = async (
  ...
  if (query.tripId) andConditions.push({ tripId: query.tripId });
  ...
  const enrichedExpenses = attachUserDetails(expenses, userMap).map((expense: any) => {
    const yourSplit = expense.splits.find((s: any) => s.userId === userId) ?? null;
    return {
      ...expense,
