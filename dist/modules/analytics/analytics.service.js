...
function buildUserMatch(userId, filters = {}) {
    const match = {
    ...
    if (filters.tripId)
        match.tripId = new mongoose_1.Types.ObjectId(filters.tripId);
    return match;
}
// ============================================================
// Analytics Service
...
exports.analyticsService = {
    ...
    async getQuickStats(userId) {
        ...
        const [thisMonth, lastMonth, thisWeek, lastWeek, today, activeTrips, pendingSettlements, topCategory, topTrip, recentExpenses, spendingTrend,] = await Promise.all([
            expense_model_1.Expense.aggregate([{ $match: { ...userFilter, date: { $gte: thisMonthStart } } }, { $group: { _id: null, total: { $sum: '$amountBase' }, count: { $sum: 1 } } }]),
            expense_model_1.Expense.aggregate([{ $match: { ...userFilter, date: { $gte: lastMonthStart, $lt: thisMonthStart } } }, { $group: { _id: null, total: { $sum: '$amountBase' }, count: { $sum: 1 } } }]),
            expense_model_1.Expense.aggregate([{ $match: { ...userFilter, date: { $gte: thisWeekStart } } }, { $group: { _id: null, total: { $sum: '$amountBase' }, count: { $sum: 1 } } }]),
            expense_model_1.Expense.aggregate([{ $match: { ...userFilter, date: { $gte: lastWeekStart, $lt: thisWeekStart } } }, { $group: { _id: null, total: { $sum: '$amountBase' }, count: { $sum: 1 } } }]),
            expense_model_1.Expense.aggregate([{ $match: { ...userFilter, date: { $gte: todayStart } } }, { $group: { _id: null, total: { $sum: '$amountBase' }, count: { $sum: 1 } } }]),
            trip_model_1.Trip.countDocuments({ 'members.userId': userId, 'members.isActive': true, status: { $in: ['active', 'planning'] }, isArchived: false }),
            expense_model_1.Expense.countDocuments({ ...userFilter, isSettled: false }),
            expense_model_1.Expense.aggregate([{ $match: { ...userFilter, date: { $gte: thisMonthStart } } }, { $group: { _id: '$category', total: { $sum: '$amountBase' } } }, { $sort: { total: -1 } }, { $limit: 1 }]),
            expense_model_1.Expense.aggregate([{ $match: { ...userFilter, date: { $gte: thisMonthStart } } }, { $group: { _id: '$tripId', total: { $sum: '$amountBase' } } }, { $sort: { total: -1 } }, { $limit: 1 }, { $lookup: { from: 'trips', localField: '_id', foreignField: '_id', as: 'trip' } }, { $unwind: { path: '$trip', preserveNullAndEmptyArrays: true } }, { $project: { tripId: '$_id', tripTitle: { $ifNull: ['$trip.title', 'Unknown'] }, total: 1 } }]),
            expense_model_1.Expense.find(userFilter).sort({ date: -1 }).limit(5).select('title amountBase amountLocal localCurrency date category paidByName').lean(),
            expense_model_1.Expense.aggregate([{ $match: { ...userFilter, date: { $gte: new Date(Date.now() - 7 * 86400000) } } }, { $group: { _id: { $dateToString: { format: '%Y-%m-%d', date: '$date' } }, amount: { $sum: '$amountBase' }, count: { $sum: 1 } } }, { $sort: { _id: 1 } }]),
        ]);
        // Get user balances
        const user = await auth_model_1.User.findById(userId).select('totalLentAcrossTrips totalOwedAcrossTrips').lean();
        ...
        return {
            ...
            spendingTrend: spendingTrend.map((s) => ({ date: s._id, amount: s.amount, count: s.count })),
        };
    },
    // ==========================================================
    ...
    async getUserAnalytics(userId, filters = {}) {
        ...
        const dateFilter = buildDateFilter(filters.startDate, filters.endDate);
        const [summaryResult, categories, monthlyData, dailyData, weeklyData, tripData, dayOfWeek, hourOfDay, highestExpense, lowestExpense,] = await Promise.all([
            expense_model_1.Expense.aggregate([{ $match: match }, { $group: { _id: null, totalSpent: { $sum: '$amountBase' }, totalExpenses: { $sum: 1 }, avgPerExpense: { $avg: '$amountBase' } } }]),
            expense_model_1.Expense.aggregate([{ $match: match }, { $group: { _id: '$category', totalAmount: { $sum: '$amountBase' }, count: { $sum: 1 }, avgAmount: { $avg: '$amountBase' } } }, { $sort: { totalAmount: -1 } }]),
            expense_model_1.Expense.aggregate([{ $match: match }, { $group: { _id: { year: { $year: '$date' }, month: { $month: '$date' } }, totalAmount: { $sum: '$amountBase' }, count: { $sum: 1 } } }, { $sort: { '_id.year': 1, '_id.month': 1 } }]),
            expense_model_1.Expense.aggregate([{ $match: { ...match, date: { $gte: new Date(Date.now() - 90 * 86400000) } } }, { $group: { _id: { $dateToString: { format: '%Y-%m-%d', date: '$date' } }, totalAmount: { $sum: '$amountBase' }, count: { $sum: 1 }, dayOfWeek: { $first: { $dayOfWeek: '$date' } } } }, { $sort: { _id: 1 } }]),
            expense_model_1.Expense.aggregate([{ $match: match }, { $group: { _id: { year: { $year: '$date' }, week: { $week: '$date' } }, totalAmount: { $sum: '$amountBase' }, count: { $sum: 1 }, startDate: { $min: '$date' }, endDate: { $max: '$date' } } }, { $sort: { '_id.year': 1, '_id.week': 1 } }]),
            ...
            expense_model_1.Expense.aggregate([{ $match: match }, { $group: { _id: { $dayOfWeek: '$date' }, totalAmount: { $sum: '$amountBase' }, count: { $sum: 1 } } }, { $sort: { _id: 1 } }]),
            expense_model_1.Expense.aggregate([{ $match: match }, { $group: { _id: { $hour: '$date' }, totalAmount: { $sum: '$amountBase' }, count: { $sum: 1 } } }, { $sort: { _id: 1 } }]),
            expense_model_1.Expense.find(match).sort({ amountBase: -1 }).limit(1).select('title amountBase amountLocal localCurrency date paidByName category').lean(),
            expense_model_1.Expense.find(match).sort({ amountBase: 1 }).limit(1).select('title amountBase amountLocal localCurrency date paidByName category').lean(),
        ]);
        const totalSpent = summaryResult[0]?.totalSpent || 0;
        const totalExpenses = summaryResult[0]?.totalExpenses || 0;
        // Median calculation
        const medianResult = await expense_model_1.Expense.aggregate([
            { $match: match },
            { $sort: { amountBase: 1 } },
        ...
        // Max daily spend
        const dailyTotals = dailyData.map((d) => ({ date: d._id, amount: d.totalAmount }));
        const maxDaily = dailyTotals.reduce((max, d) => d.amount > (max?.amount || 0) ? d : max, dailyTotals[0] || { date: '', amount: 0 });
        // Period comparison
        ...
        if (filters.compareWith && filters.startDate && filters.endDate) {
            ...
            const [prevResult] = await expense_model_1.Expense.aggregate([{ $match: prevMatch }, { $group: { _id: null, totalSpent: { $sum: '$amountBase' }, totalExpenses: { $sum: 1 } } }]);
        ...
        return {
            ...
            categories: categories.map((c) => ({
                category: c._id,
                emoji: analytics_types_1.CATEGORY_EMOJIS[c._id] || '📌',
            ...
            dailySpending: dailyData.map((d) => ({
                date: d._id,
                totalAmount: d.totalAmount,
                count: d.count,
                dayOfWeek: DAY_NAMES[(d.dayOfWeek - 1) % 7],
            })),
            weeklySpending: weeklyData.map((w) => ({
            ...
            dayOfWeekPattern: DAY_NAMES.map((day, index) => {
                const found = dayOfWeek.find((d) => d._id === index + 1);
                return {
                    day,
            ...
            hourOfDayPattern: Array.from({ length: 24 }, (_, i) => {
                const found = hourOfDay.find((h) => h._id === i);
    ...
    // ==========================================================
    async getTripAnalytics(tripId, userId, filters = {}) {
        ...
        const match = { tripId: new mongoose_1.Types.ObjectId(tripId) };
        const dateFilter = buildDateFilter(filters.startDate, filters.endDate);
        if (dateFilter)
            match.date = dateFilter;
        if (filters.category)
            match.category = filters.category;
        if (filters.stopId)
            match.stopId = new mongoose_1.Types.ObjectId(filters.stopId);
        const [summaryResult, categories, dailyData, stopData, currencyData, settlementStats, dayOfWeek, highestExpense, lowestExpense, spendingVelocity,] = await Promise.all([
            expense_model_1.Expense.aggregate([{ $match: match }, { $group: { _id: null, totalSpent: { $sum: '$amountBase' }, totalExpenses: { $sum: 1 }, avgPerExpense: { $avg: '$amountBase' } } }]),
            expense_model_1.Expense.aggregate([{ $match: match }, { $group: { _id: '$category', totalAmount: { $sum: '$amountBase' }, count: { $sum: 1 }, avgAmount: { $avg: '$amountBase' } } }, { $sort: { totalAmount: -1 } }]),
            expense_model_1.Expense.aggregate([{ $match: match }, { $group: { _id: { $dateToString: { format: '%Y-%m-%d', date: '$date' } }, totalAmount: { $sum: '$amountBase' }, count: { $sum: 1 } } }, { $sort: { _id: 1 } }]),
            expense_model_1.Expense.aggregate([{ $match: match }, { $group: { _id: '$stopId', totalLocal: { $sum: '$amountLocal' }, totalBase: { $sum: '$amountBase' }, count: { $sum: 1 }, currency: { $first: '$localCurrency' } } }, { $sort: { totalBase: -1 } }]),
            expense_model_1.Expense.aggregate([{ $match: match }, { $group: { _id: '$localCurrency', totalLocal: { $sum: '$amountLocal' }, totalBase: { $sum: '$amountBase' }, count: { $sum: 1 }, exchangeRate: { $avg: '$exchangeRateUsed' } } }]),
            expense_model_1.Expense.aggregate([{ $match: match }, { $group: { _id: '$isSettled', count: { $sum: 1 }, totalAmount: { $sum: '$amountBase' } } }]),
            expense_model_1.Expense.aggregate([{ $match: match }, { $group: { _id: { $dayOfWeek: '$date' }, totalAmount: { $sum: '$amountBase' }, count: { $sum: 1 } } }, { $sort: { _id: 1 } }]),
            expense_model_1.Expense.find(match).sort({ amountBase: -1 }).limit(1).select('title amountBase amountLocal localCurrency date paidByName category').lean(),
            expense_model_1.Expense.find(match).sort({ amountBase: 1 }).limit(1).select('title amountBase amountLocal localCurrency date paidByName category').lean(),
            expense_model_1.Expense.aggregate([{ $match: match }, { $sort: { date: 1 } }, { $group: { _id: { $dateToString: { format: '%Y-%m-%d', date: '$date' } }, dailyAmount: { $sum: '$amountBase' } } }, { $sort: { _id: 1 } }]),
        ]);
        const totalSpent = summaryResult[0]?.totalSpent || 0;
        const totalExpenses = summaryResult[0]?.totalExpenses || 0;
        const settled = settlementStats.find((s) => s._id === true);
        const unsettled = settlementStats.find((s) => s._id === false);
        const tripDays = Math.max(1, Math.ceil((trip.endDate.getTime() - trip.startDate.getTime()) / 86400000));
        // Median
        const medianResult = await expense_model_1.Expense.aggregate([{ $match: match }, { $sort: { amountBase: 1 } }, { $group: { _id: null, amounts: { $push: '$amountBase' } } }]);
        const amounts = medianResult[0]?.amounts || [];
        const median = amounts.length > 0 ? amounts[Math.floor(amounts.length / 2)] : 0;
        // Max daily
        const dailyTotals = dailyData.map((d) => ({ date: d._id, amount: d.totalAmount }));
        ...
        // Stop breakdown
        const stopBreakdown = stopData.map((s) => {
            const stop = trip.stops.find((st) => st._id.toString() === s._id.toString());
    ...
    async getYearlySummary(userId, year) {
        ...
        const [monthlyData, categoryData, quarterlyData, prevYearData] = await Promise.all([
            expense_model_1.Expense.aggregate([{ $match: match }, { $group: { _id: { $month: '$date' }, totalAmount: { $sum: '$amountBase' }, count: { $sum: 1 } } }, { $sort: { _id: 1 } }]),
            expense_model_1.Expense.aggregate([{ $match: match }, { $group: { _id: '$category', totalAmount: { $sum: '$amountBase' }, count: { $sum: 1 } } }, { $sort: { totalAmount: -1 } }]),
            expense_model_1.Expense.aggregate([{ $match: match }, { $group: { _id: { $ceil: { $divide: [{ $month: '$date' }, 3] } }, totalAmount: { $sum: '$amountBase' }, count: { $sum: 1 }, months: { $addToSet: { $month: '$date' } } } }, { $sort: { _id: 1 } }]),
            expense_model_1.Expense.aggregate([{ $match: { ...match, date: { $gte: new Date(`${year - 1}-01-01`), $lte: new Date(`${year - 1}-12-31`) } } }, { $group: { _id: null, totalSpent: { $sum: '$amountBase' }, totalExpenses: { $sum: 1 } } }]),
        ]);
