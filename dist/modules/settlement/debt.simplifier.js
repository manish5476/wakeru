...
exports.debtSimplifier = exports.DebtSimplifier = void 0;
...
class DebtSimplifier {
    ...
    simplifyDebts(debts) {
        ...
        debts.forEach(debt => {
            ...
            const to = debt.to;
            const amount = new decimal_js_1.default(debt.amount.toString());
            // User who owes money (negative balance)
            const fromBalance = netBalances.get(from) || new decimal_js_1.default(0);
            netBalances.set(from, fromBalance.minus(amount));
            // User who is owed money (positive balance)
            const toBalance = netBalances.get(to) || new decimal_js_1.default(0);
        ...
        while (creditorIdx < creditors.length && debtorIdx < debtors.length) {
            ...
            const transferAmount = decimal_js_1.default.min(creditor.amount, debtor.amount);
            if (transferAmount.greaterThan(0.01)) {
                simplifiedTransactions.push({
                    from: debtor.userId,
                    to: creditor.userId,
                    amount: mongoose_1.Types.Decimal128.fromString(transferAmount.toFixed(2)),
     ...
     */
    applyCircularOptimization(transactions) {
        ...
        for (let i = 0; i < transactions.length; i++) {
            ...
            for (let j = i + 1; j < transactions.length; j++) {
                ...
                if (tx1.from === tx2.to && tx1.to === tx2.from) {
                    const amount1 = new decimal_js_1.default(tx1.amount.toString());
                    const amount2 = new decimal_js_1.default(tx2.amount.toString());
                    if (amount1.equals(amount2)) {
                        ...
                        optimized.push({
                            ...tx1,
                            amount: mongoose_1.Types.Decimal128.fromString(diff.toFixed(2))
                        });
                        processed.add(key1);
                        processed.add(key2);
                        foundOptimization = true;
     ...
     */
    getDebtSummary(userId, debts) {
        const owes = new Map();
        const isOwed = new Map();
        debts.forEach(debt => {
            if (debt.from === userId) {
...
exports.DebtSimplifier = DebtSimplifier;
exports.debtSimplifier = new DebtSimplifier();
//# sourceMappingURL=debt.simplifier.js.map