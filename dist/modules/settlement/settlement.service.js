...
const calculateSettlement = async (tripId, requestingUid) => {
    ...
    const receivers = await auth_model_1.User.find({
        ...
        .lean();
    const upiMap = new Map(receivers.map((r) => [r.firebaseUid, r.bankingDetails?.upiId]));
    ...
    const existingSettlement = await settlement_model_1.Settlement.findOne({
        tripId: new mongoose_1.Types.ObjectId(tripId),
    }).lean();
