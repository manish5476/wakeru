...
// ============================================================
/**
 * Given a list of firebaseUids, fetch a lightweight public profile for each
 * from the User collection and return a lookup map.
 *
 * Kept intentionally small/public-safe (no email, phone, banking info) since
 * this data gets embedded into expenses returned to *other* trip members too.
 */
const buildUserSummaryMap = async (firebaseUids) => {
    const uniqueUids = [...new Set(firebaseUids)].filter(Boolean);
    if (uniqueUids.length === 0)
        return new Map();
    const users = await auth_1.User.find({ firebaseUid: { $in: uniqueUids }, isDeleted: false }, { firebaseUid: 1, displayName: 1, photoURL: 1, bio: 1 }).lean();
    const map = new Map();
...
/**
 * Attach `payer` (full user summary for paidBy) and `user` (per-split user
 * summary) onto each expense. Falls back to the denormalized name already
 * stored on the expense if the user record is missing/deleted.
 */
const attachUserDetails = (expenses, userMap) => {
...
const getMyExpenses = async (userId, query) => {
    ...
    // Explicit paidBy filter (e.g. "show me what Alice paid") is preserved
    // even when `type` is also present — combined via $and rather than
    // overwritten.
    if (query.paidBy)
        andConditions.push({ paidBy: query.paidBy });
    if (query.startDate || query.endDate) {
        const dateFilter = {};
        if (query.startDate)
    ...
    // Base involvement filter: the requesting user must be either the payer
    // or a split participant, unless a more specific `type` narrows it.
    ...
    switch (query.type) {
        case 'you_owe':
            ...
            andConditions.push({ splits: { $elemMatch: { userId, isPaid: false } } });
            break;
        case 'you_paid':
    ...
    const involvedUids = expenses.flatMap((e) => [
        e.paidBy,
        ...((e.splits ?? []).map((s) => s.userId)),
    ]);
    const userMap = await buildUserSummaryMap(involvedUids);
    const enrichedExpenses = attachUserDetails(expenses, userMap).map((expense) => {
        const yourSplit = expense.splits.find((s) => s.userId === userId) ?? null;
        return {
            ...expense,
...
const markSplitPaid = async (expenseId, targetUserId, requestingUid, paymentId) => {
    ...
    await expense.save();
    // FIX: this mutation changes isPaid / isSettled but previously never
    // invalidated the settlement cache the way every other mutation does.
    await (0, settlement_service_1.markSettlementStale)(expense.tripId.toString());
    return expense;
};
