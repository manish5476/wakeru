...
class IdempotencyMiddleware {
    static async checkIdempotency(req, res, next) {
        try {
            ...
            if (!idempotencyKey) {
                throw new AppError_1.AppError('Idempotency-Key header is required for mutation operations', 400, 'IDEMPOTENCY_KEY_MISSING');
            }
            const uuidRegex = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
            if (!uuidRegex.test(idempotencyKey)) {
                throw new AppError_1.AppError('Invalid Idempotency-Key format. Must be UUID v4', 400, 'INVALID_IDEMPOTENCY_KEY');
            }
